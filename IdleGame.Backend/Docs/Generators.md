...
3. Generator Definitions (Design‑Time Data)
	Generators are defined in JSON files and loaded at startup.

	Each generator definition includes:
...
6. Cost Scaling Rules
	Generator upgrade cost scales exponentially: Cost(level) = BaseUpgradeCost × (UpgradeCostMultiplier ^ (level - 1))
